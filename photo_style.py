...
from neuralartweb.api_v0.utils import save_result as neuralartweb_save_result
...
def save_result(args, img_, str_, iteration=None):
    result = Image.fromarray(np.uint8(np.clip(img_, 0, 255.0)))
    result.save(str_)
    # Neural Art Webを利用するため
    if hasattr(args, "material_detail"):
        neuralartweb_save_result(args.material_detail, str_, iteration=iteration or -1)
...
def print_loss(args, loss_content, loss_styles_list, loss_tv, loss_affine, overall_loss, output_image):
    ...
    if iter_count % args.save_iter == 0 and iter_count != 0:
        save_result(args,
                    best_image[:, :, ::-1],
                    os.path.join(args.serial, 'out_iter_{}.png'.format(iter_count)),
    ...
    if args.lbfgs:
        ...
        for i in xrange(1, args.max_iter):
            ...
            if i % args.save_iter == 0 and i != 0:
                save_result(args,
                            best_image[:, :, ::-1],
                            os.path.join(args.serial, 'out_iter_{}.png'.format(i)),
                            iteration=i)
